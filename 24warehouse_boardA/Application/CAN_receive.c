...
bool_t can_reset_flag[5]={0};			//单个电机里程计归零的标志

static CAN_TxHeaderTypeDef  chassis_tx_message;
static CAN_TxHeaderTypeDef  bodanpan_tx_message;
static uint8_t chassis_can_send_data[8];
static uint8_t bodanpan_can_send_data[2];
/**
...
void CAN_cmd_pan(int16_t motor_trigger)
{
	...
	bodanpan_tx_message.StdId = CAN_TRIGGER_ALL_ID;	//给5-8号电机发送报文时用的标识符
	bodanpan_tx_message.IDE = CAN_ID_STD;
	bodanpan_tx_message.RTR = CAN_RTR_DATA;
	bodanpan_tx_message.DLC = 0x08;
  
	bodanpan_can_send_data[0] = motor_trigger >> 8;
	bodanpan_can_send_data[1] = motor_trigger;
	HAL_CAN_AddTxMessage(&hcan1, &bodanpan_tx_message, bodanpan_can_send_data, &send_mail_box);
}
