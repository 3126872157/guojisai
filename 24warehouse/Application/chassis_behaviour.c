...
//稍后完善
bool_t flag_can[4];		//这是啥
bool_t code_reset_flag;	//里程计清零？
//extern bool_t flag_can[4];		//这是啥
//extern bool_t code_reset_flag;	//里程计清零？

//这个是啥？？？？？？？？？？
void RESET_CODE(chassis_motor_t *reset_code)
{
...
static void  chassis_infrared_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_vector)
{
    if (vx_set == NULL || vy_set == NULL || wz_set == NULL || chassis_move_vector == NULL)
    {
        return;
			...
			if(code_reset_flag==1)
		{
			for(uint8_t i=0;i<=3;i++)
			{
				RESET_CODE(&chassis_move_vector->motor_chassis[i]);
			}
