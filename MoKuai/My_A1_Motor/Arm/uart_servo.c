...
extern uart_servo_Data_1 uart_servo_Data;  //接收数据
...
/*********************************************************************************
 * Function:  uart_servo_UART5_Init
 * Description： 初始换串口（空闲中断）
 * Parameters:   1.接收区     2.接收区长度（大于等于数据长度）
 * Return:       无返回
 * Others:
 **********************************************************************************/
void uart_servo_UART5_Init(uint8_t* rxBuf,uint8_t len)
{
	...
	HAL_UART_Receive_DMA(&huart5, LobotRxBuf, 255);     //设置DMA传输，将串口1的数据搬运到recvive_buff中，每次255个字节
}
...
void moveServo(uint8_t servoID, uint16_t Position, uint16_t Time)
{
	...
	HAL_UART_Transmit_DMA(&huart5,LobotTxBuf, 10);
}

/*********************************************************************************
 * Function:  moveServos
 * Description： 得到多个舵机角度数据
 * Parameters:   Num:舵机个数,舵机ID,舵机ID
 * Return:       无返回
 * Others:
 **********************************************************************************/
...
{
	LobotTxBuf[0] = LobotTxBuf[1] = FRAME_HEADER;      //填充帧头
	LobotTxBuf[2] = 5;                //数据长度 = 要读取舵机数 + 3
	LobotTxBuf[3] = CMD_MULT_SERVO_POS_READ;             //舵机读取指令
	LobotTxBuf[4] = 2;                        //要读取舵机数
	LobotTxBuf[5] = 1;                        //要读取舵机数
	LobotTxBuf[6] = 2; 

	HAL_UART_Transmit_DMA(&huart5,LobotTxBuf, 7);    //发送
}

/*********************************************************************************
 * Function:  CaluServosGyro
 * Description： 计算多个舵机角度数据
 * Parameters:   无
 * Return:       无返回
 * Others:
 **********************************************************************************/
...
{
	if(uart_servo_Data.LobotRxData[5] == 1)
	{
		uart_servo_Data.servo_uart_W1 = (uart_servo_Data.LobotRxData[6] | (uart_servo_Data.LobotRxData[7]<<8));
	}
		if(uart_servo_Data.LobotRxData[8] == 2)
	{
		uart_servo_Data.servo_uart_W2  = (uart_servo_Data.LobotRxData[9] | (uart_servo_Data.LobotRxData[10]<<8));
	}
}
...
//	HAL_UART_Transmit_DMA(&huart5,LobotTxBuf, LobotTxBuf[2] + 2);             //发送
//}
...
//	HAL_UART_Transmit_DMA(&huart5,LobotTxBuf, 4);      //发送
//}
///*********************************************************************************
