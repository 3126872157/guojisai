...
//#pragma pack(1) //结构体，共用体内存分配以1字节对齐
...
//typedef struct
//{
//    uint8_t start[2]; // 包头，start[0]为0xFE，start[1]为0xEE
//    uint8_t motorID;  // 电机ID  0,1,2,0xBB；0xBB 表示向所有电机广播（此时电机无返回数据）
...
//{                      // 以 4个字节一组排列 ，不然编译器会凑整
//                       // 定义 数据
//    uint8_t mode;      // 关节模式选择，0停转，5开环慢转，10闭环伺服控制
//    uint8_t ModifyBit; // 电机内部控制参数修改位，可忽略
//    uint8_t ReadBit;   // 电机内部控制参数发送位，可忽略
//    uint8_t reserved;  // 预留位，可忽略

//    COMData32 Modify;  // 电机参数修改的数据，可忽略
//	
//    //PD控制器混控：实际给FOC的指令力矩为：
//    // K_P*delta_Pos + K_W*delta_W + T(前馈力矩)
//    int16_t T;         // 期望关节的力矩（电机本身的力矩）x256     7 + 8 描述
//    int16_t W;         // 期望关节角速度（电机本身的速度）x128     8 + 7 描述
//    int32_t Pos;       // 期望关节位置 x 16384/(2 x pi), 14位编码器（主控0点修正，电机关节还是以编码器0点为准）
...
//} MasterComdV3;        // 加上数据包的包头和CRC 共34字节
...
//    // 定义 数据
//    uint8_t mode;    // 当前关节模式
//    uint8_t ReadBit; // 表示电机控制参数修改是否成功，可忽略
...
//    float LW;        // 当前实际电机速度（低速）   经滤波，有延迟

//    int16_t W2;      // 当前实际关节速度（高速）   8 + 7 描述，为关节编码器预留，可忽略
//    float LW2;       // 当前实际关节速度（低速）   为关节编码器预留，可忽略

//    int16_t Acc;     // 电机转子加速度，x1描述     15+0 描述  惯量较小
//    int16_t OutAcc;  // 输出轴加速度              12+3 描述  惯量较大，可忽略

//    int32_t Pos;     // 当前电机位置（主控0点修正，电机关节还是以编码器0点为准）
//    int32_t Pos2;    // 关节编码器位置(输出编码器)

//    int16_t gyro[3]; // 电机驱动板6轴传感器数据，分别为x,y,z轴角速度，x 2000 / (2 ^ 15) x (2 x PI) / 360 换算为 rad/s
//    int16_t acc[3];  // 电机控制板上IMU在x,y,z轴上的线加速度，x 8 x 9.80665 / (2 ^ 15) 后换算为 m/ss
