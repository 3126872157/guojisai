...
void tulun_control(bool_t is_up)
{
	if(is_up)
	{
		__HAL_TIM_SetCompare(&PWM_SERVO_TIM, TULUN_CHANNEL, tulun_up_pwm);//1250升起250落下
	}
	else
	{
		__HAL_TIM_SetCompare(&PWM_SERVO_TIM, TULUN_CHANNEL, tulun_down_pwm);
	}
...
void bogan_control(uint8_t mode)
{
	...
	{
		__HAL_TIM_SetCompare(&PWM_SERVO_TIM, BOGAN_CHANNEL, bogan_jiqiu_pos);//800
	}
	else if(mode == 2)	//准备击球
	{
